...
if [ ! -d "$SOURCE_PATH" ]; then
    ...
    echo "💡 Rappel : Si c'est un partage réseau (\\\\192.168.1.100\\...), montez-le d'abord dans /mnt/books"
    exit 1
fi
...
if [ -z "$(ls -A "$SOURCE_PATH" 2>/dev/null)" ]; then
    ...
    echo "💡 Vérifiez que votre partage réseau Samba/NAS (//192.168.1.100/public/misc/BOOKS) est accessible."
    exit 1
fi
